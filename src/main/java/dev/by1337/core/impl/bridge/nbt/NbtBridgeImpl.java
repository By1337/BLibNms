...
public class NbtBridgeImpl implements NbtBridge {
        ...
        return switch (tag.getId()) {
            ...
            case TAG_COMPOUND -> {
                ...
                for (Map.Entry<String, BinaryNbt.NbtTag> entry : ((BinaryNbt.CompoundTag) tag).tags().entrySet()) {
                    compoundTag.set(entry.getKey(), (Tag) toNMS(entry.getValue()));
                }
                yield compoundTag;
            }
    ...
    public BinaryNbt.NbtTag ofNMS(Object tag) {
        ...
        return switch (nms.getTypeId()) {
            ...
            case TAG_BYTE -> new BinaryNbt.ByteTag(((ByteTag) nms).asByte());
            case TAG_SHORT -> new BinaryNbt.ShortTag(((ShortTag) nms).asShort());
            case TAG_INT -> new BinaryNbt.IntTag(((IntTag) nms).asInt());
            case TAG_LONG -> new BinaryNbt.LongTag(((LongTag) nms).asLong());
            case TAG_FLOAT -> new BinaryNbt.FloatTag(((FloatTag) nms).asFloat());
            case TAG_DOUBLE -> new BinaryNbt.DoubleTag(((DoubleTag) nms).asDouble());
            case TAG_STRING -> new BinaryNbt.StringTag(nms.asString());
            case TAG_BYTE_ARRAY -> {
                byte[] v = ((ByteArrayTag) nms).getBytes();
                yield new BinaryNbt.ByteArrayTag(v);
            }
            case TAG_INT_ARRAY -> {
                int[] v = ((IntArrayTag) nms).getInts();
                yield new BinaryNbt.IntArrayTag(v);
            }
            case TAG_LONG_ARRAY -> {
                long[] v = ((LongArrayTag) nms).getLongs();
